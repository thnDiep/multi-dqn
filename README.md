...
| `expert/attention_network.py` | Attention model definitions (GFA, LFA, TFA) and model builder |
...
- **TimeFrameAttention (TFA)**: computes a scalar importance score for each temporal resolution via separate projection weights, produces a 3-way softmax over timeframes, scales each segment by its gate weight, and then applies a final sigmoid gate over the weighted representation.
...
Results are written to `./Output/results/<market>/`.
